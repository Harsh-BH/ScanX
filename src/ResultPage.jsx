...
function ResultPage() {
  ...
  // Check if the result is from an image (i.e., no frame_number or timestamp exists)
  const isImage = result.details.every(frame => frame.frame_number === null && frame.timestamp === null);
  ...
  >
    <div className="min-h-screen to-black text-white p-10 w-full">
      {/* Header */}
              ...
              {result.details.map((frame, index) => (
                <tr key={index} className="border-b border-white/20">
                  <td className="py-2 px-6">
                    {isImage ? 'N/A' : frame.frame_number ?? 'N/A'}
                  </td>
                  <td className="py-2 px-6">
                    {isImage ? 'N/A' : frame.timestamp !== null ? `${frame.timestamp.toFixed(2)}s` : 'N/A'}
                  </td>
